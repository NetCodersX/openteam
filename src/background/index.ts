...
function senderFrameId(sender: chrome.runtime.MessageSender): number | undefined {
  return sender.frameId && sender.frameId > 0 ? sender.frameId : undefined
}
...
async function pushHost(message: BackgroundToHostMessage): Promise<void> {
  ...
  try {
    await chrome.runtime.sendMessage({ type: 'OPENTEAM_HOST_PUSH', payload: message })
  } catch (error) {
    log.debug('push-host-runtime:failed', {
      type: message.type,
      error: error instanceof Error ? error.message : String(error),
    })
  }
...
async function sendRoleMessage(tabId: number, message: BackgroundToRoleMessage, frameId?: number): Promise<unknown> {
...
async function handleHostReady(message: Extract<HostToBackgroundMessage, { type: 'TEAM_HOST_READY' }>, sender: chrome.runtime.MessageSender) {
  const tabId = message.hostTabId ?? senderTabId(sender)
  if (tabId === undefined) return { ok: false, error: 'Missing sender tab' }
...
async function handleCreateRole(message: Extract<HostToBackgroundMessage, { type: 'TEAM_CREATE_ROLE' }>, sender: chrome.runtime.MessageSender) {
  ...
  if (message.container === 'iframe') {
    const tabId = message.hostTabId ?? senderTabId(sender)
    if (tabId === undefined) return { ok: false, error: 'Missing sender tab' }

    room.setHostTab(tabId)
    const role = room.addOpeningRole(name, tabId, '__pending__', now())
    log.info('iframe-role-created', { roleId: role.id, roleName: role.name, tabId })
    await pushState()
    return { ok: true, role }
  }
...
async function handleRemoveRole(message: Extract<HostToBackgroundMessage, { type: 'TEAM_REMOVE_ROLE' }>) {
  ...
  if (role && role.frameId === undefined) {
    try {
...
async function handleRoleStatus(message: Extract<RoleToBackgroundMessage, { type: 'TEAM_ROLE_STATUS' }>, sender: chrome.runtime.MessageSender) {
  ...
  room.markRoleStatus(tabId, message.status, now(), message.error, frameId)
  log.info('role-status', { tabId, frameId, status: message.status, error: message.error })
  if (message.status === 'generating' && frameId === undefined) {
    renderWakeScheduler.schedule(tabId, getState().hostTabId)
    log.info('render-wake:scheduled', { tabId, hostTabId: getState().hostTabId })
  }
  if (frameId === undefined && (message.status === 'idle' || message.status === 'error' || message.status === 'offline')) {
...
async function handleRoleReply(message: Extract<RoleToBackgroundMessage, { type: 'TEAM_ROLE_REPLY' }>, sender: chrome.runtime.MessageSender) {
  ...
  const reply = frameId === undefined
    ? room.recordRoleReply(tabId, message.content, now(), message.messageId)
    : room.recordRoleReplyFromFrame(tabId, frameId, message.content, now(), message.messageId)
  if (frameId === undefined) renderWakeScheduler.cancel(tabId)
  log.info('role-reply', {
    tabId,
    frameId,
...
async function handleFrameRoleReady(message: Extract<RoleToBackgroundMessage, { type: 'TEAM_FRAME_ROLE_READY' }>, sender: chrome.runtime.MessageSender) {
  const tabId = senderTabId(sender)
  const frameId = senderFrameId(sender)
  if (tabId === undefined || frameId === undefined) return { ok: false, error: 'Missing sender frame' }

  const readyRole = room.markRoleFrameReady(message.roleId, tabId, frameId, message.conversationId, now())
  if (!readyRole) return { ok: false, error: 'Unknown role' }
  ...
  log.info('iframe-role-ready', { tabId, frameId, roleId: readyRole.id, roleName: readyRole.name })
  await pushState()
  return { ok: true, mode: 'role', role: readyRole }
}
  ...
  const run = async () => {
    switch (message?.type) {
      ...
      case 'TEAM_HOST_READY':
        return handleHostReady(message, sender)
      case 'TEAM_GET_STATE':
        return { ok: true, state: getState() }
      ...
      case 'TEAM_FRAME_ROLE_READY':
        return handleFrameRoleReady(message, sender)
      default:
