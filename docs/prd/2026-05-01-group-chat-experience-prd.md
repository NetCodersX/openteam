...
本 PRD 第 6 节列出的功能需求和第 10 节验收清单均为本轮必须实现的范围。后文如出现优先级表述，只表示建议实施顺序，不表示可以裁剪或延期。
...
```text
iframe-host   背景层，承载 Gemini iframe 群组工作区
app-shell     前景层，承载 OpenTeam 群聊悬浮窗
```

`iframe-host` 负责展示和保留各群聊对应的 Gemini iframe。`app-shell` 只承载群聊列表、消息流、输入框、人员抽屉、设置菜单和弹窗，不承载 iframe 卡片。

切换群聊时，已创建的 iframe 和 iframe group 必须继续保留在背景工作区中，不应销毁，也不应通过 `hidden` / `display: none` 等方式隐藏。当前群聊可以在视觉上更突出，非当前群聊可以弱化、缩略或排布到旁边，但仍然需要持续存在。
...
- 人设必填，不允许为空。
- 描述可选，允许较长内容。
- 如果某个人员库人员已经被任一群聊使用，不允许删除该人员库人员。
- 人员库人员被加入群聊后，会复制为群聊内独立人员；之后人员库信息变更不自动影响已加入群聊的人员。
...
- 从人员库添加或临时添加后，人设不可在当前群聊内继续编辑。
- 临时人员只存在于当前群聊，不允许保存回人员库。
- 群聊内人员可以支持头像颜色编辑；颜色变更只影响当前群聊内人员实例，不影响人员库模板。
- 人设只应在初始化、重新唤醒或创建新的 Gemini 会话时发送；普通消息不应每次重复携带完整人设，避免上下文暴涨。
...
是否持久化该占位消息可以由实现决定。推荐第一版不持久化，而是根据人员状态 `thinking` 动态渲染。Assistant 回复不要求关联触发它的 user message；同一个人员同一时间只处理一个消息，因此同一个人员最多展示一个正在回复气泡。
...
- 人设必填，不允许为空。
- 描述可选，允许较长内容。
- 如果人员库人员已经被任一群聊使用，不允许删除；删除按钮应禁用或在点击后给出明确提示。
...
- 支持从人员库批量选择人员。
- 批量添加应走批量创建 API，避免 UI 逐条循环创建导致多次刷新和部分成功状态不清晰。
- 新人员有独立 `id`。
- 新人员绑定当前 `chatId`。
- 不共享人员库的会话状态。
- 不共享其他群聊里的同名人员状态。
...
临时人员只加入当前群聊，不写入人员库，不出现在人员库列表中，也不支持保存回人员库。临时人员加入后，人设不允许继续编辑；后续可以支持编辑头像颜色，颜色只影响当前群聊内的该人员。
...
- 从人员库批量添加时，应一次性创建多个群聊内人员。

## 6.4 右侧人员面板默认收起

### 描述

右侧当前群聊人员区域默认完全收起，只显示摘要入口。点击摘要后，以右侧抽屉形式展开人员详情。展开状态不按群聊记忆，每次进入默认收起。
...
当人员状态进入 `thinking` 时，当前群聊消息流展示占位气泡。同一个人员同一时间只处理一个消息，因此同一个人员最多展示一个正在回复气泡。
...
- 单个人员同一时间最多显示一个正在回复气泡。
- 超过 120 秒未收到回复时，正在回复气泡消失并展示超时或异常状态。
...
- 代码块。
- 代码块语法高亮。
- HTML / JavaScript / TypeScript / CSS / JSON / Bash 等常见代码展示。
- 有序列表。
- 无序列表。
- 引用块。
- 链接。
...
- Markdown 可以展示 HTML / JavaScript 等代码内容，但不允许执行 Markdown 中的脚本。
- 不允许执行 `<script>`、内联事件处理器或 `javascript:` 链接。
- 外链打开策略应安全。
- 渲染器需避免 XSS。

### 实现建议

可以使用 `markdown-it` 或同类成熟库，并配置安全渲染策略。代码高亮可以使用 `highlight.js`、`shiki` 或等价方案。
...
用户可以通过左侧群聊列表中每个群聊项旁边的“三点菜单”编辑群名称。不要使用浏览器右键菜单，避免和浏览器原生功能冲突。
...
- 用户切到群聊 B。
- 群聊 A 的 iframe 和 iframe group 仍保留在背景工作区，不销毁、不隐藏。
...
- 切换群聊不会销毁原群聊 iframe。
- 切换群聊不会通过 `hidden` / `display: none` 隐藏已存在 iframe。
- 非当前群聊收到回复后，消息可以持久化。
- 刷新页面后，已收到消息仍然存在。
...
这张图用于说明页面分层：`team.html` 的背景层是大屏 `iframe-host`，其中按群聊创建 `chat-frame-group`；前景层是 OpenTeam 群聊悬浮窗，也就是 `app-shell`，只负责群聊列表、消息流、人员摘要和输入。当前群聊的 iframe group 可以在背景层放大显示，非当前群聊 group 保持存在，可以弱展示、缩略展示或排布到旁边，但不销毁、不隐藏；所有运行态绑定都以 `chatId + personId` 为粒度，避免不同群聊的同名人员串上下文。
...
- 非当前群聊的 iframe group 不能销毁，也不能隐藏；可以弱展示、缩略展示或排布到旁边。
- OpenTeam 群聊悬浮窗不承载 iframe 卡片，只承载群聊 UI。
- 用户想查看原始 Gemini 页面时，可以缩小或移开群聊悬浮窗，直接操作背景层 iframe group。
- 切换群聊只切换前景消息流、标题、人员摘要和背景层当前 group 的视觉突出状态，不重建 iframe。
...
- 非当前群聊 iframe 仍保留在背景工作区，继续运行和上报回复。
...
左侧不显示未读数字，只显示“有新消息”状态点或状态标记。状态优先级建议为：异常 > 回复中 > 有新消息。

### 验收标准

- 非当前群聊收到新回复后，左侧列表有明显但克制的“有新消息”提示。
- 不显示未读数字。
- 用户切换到该群聊后，“有新消息”提示消失。
- 回复中状态不依赖当前选中群聊。
...
用户新建群聊时，必须能够明确选择群聊模式，不能由左侧加号直接按默认值创建成“独立专家”模式。当前创建群聊能力已基本实现，后续除测试修复和验收补齐外，不主动调整该流程。
...
左侧群聊列表不显示未读数字，只显示是否有新消息。右侧人员面板默认收起，不需要按群聊持久化展开状态。
...
- `markdown-it`
- `highlight.js` / `shiki` 或等价代码高亮库
- 或等价轻量库

必须处理安全问题：

- 可以展示 HTML / JavaScript 代码，但不能执行脚本。
...
- 如果 120 秒内没有收到回复，移除 thinking 并展示超时或异常状态。
...
- @ 人员头像只显示首字。
- 人员回复前显示动态正在回复气泡，超时默认 120 秒。
- Markdown 渲染稳定，代码块支持语法高亮。
...
本 PRD 第 6 节列出的功能需求全部属于必须完成范围，不再按 P0 / P1 区分是否交付。下面只表示建议实施顺序：

1. 验证并保留已实现的新建群聊模式选择。
2. 文案统一为人员体系。
3. 设置小菜单 + 人员库弹窗。
4. 人员库删除保护：已被任一群聊使用的人员库人员不允许删除。
5. 添加人员弹窗：从人员库批量添加 / 临时添加。
6. 右侧人员区默认收起，并以右侧抽屉展开。
7. Enter 发送、Command/Ctrl + Enter 换行。
8. @ 头像只显示首字。
9. 正在回复动态气泡，默认 120 秒超时。
10. 群聊三点菜单改名。
11. Markdown 渲染统一，代码块支持语法高亮。
12. 多群聊后台并行体验补齐。
13. iframe 按群聊分组，并在背景工作区持续保留，不隐藏已存在 group。
14. 左侧群聊后台活动和有新消息提示。
15. 缩小为右下角圆形入口。
16. 修复普通消息每次重复携带完整人设的问题：人设只在初始化、重新唤醒或新建会话时发送。

后续规划，不属于本轮第 6 节核心功能需求：
...
切换群聊时不应隐藏或卸载已有 iframe。实现时需要让已创建的 iframe group 持续保留在背景工作区，可通过弱展示、缩略展示或旁侧排布降低视觉干扰，并通过现有 content script 上报机制确认回复仍可到达。

### 12.2 Markdown 安全风险

Markdown 需要展示代码块、HTML / JavaScript 代码和语法高亮，但不能执行 Markdown 中的脚本。不能直接把 Markdown 渲染后的 HTML 无过滤插入页面，需要禁用脚本执行或做安全清理，避免 XSS。
...
- 本 PRD 第 6 节列出的功能需求均必须实现。
- 人员库人员只是人设模板。
- 加入群聊后生成独立人员。
- 临时添加人员不进入人员库，也不允许保存回人员库。
- 人员名称最多 10 个字，人设必填，描述可较长。
- 从人员库添加人员支持批量选择，并走批量创建 API。
- 如果人员库人员已经被任一群聊使用，不允许删除。
- 群聊内人员人设不可编辑，只能查看。
- 人设只在初始化、重新唤醒或新建会话时发送，普通消息不重复携带完整人设。
- 多群聊后台并行。
- 切换群聊时已存在 iframe group 不销毁、不隐藏。
- 右侧人员区默认完全收起，展开形态为右侧抽屉，不按群聊记忆。
- 设置入口先展示小菜单，再进入人员库弹窗。
- 正在回复气泡默认 120 秒超时。
- @ 人员逻辑保持现状，只修头像首字显示和换行问题。
- 群聊名称编辑使用三点菜单，不使用右键。
- 左侧群聊有新消息提示只显示状态点，不显示未读数字。
- Markdown 支持代码高亮和 HTML / JavaScript 代码展示，但不执行脚本。
- 当前新建群聊模式选择已实现，后续除测试修复和验收补齐外不主动调整。
