...
describe('IframeHost', () => {
  ...
  it('keeps a low-frequency assignment heartbeat after a role is ready', () => {
    const visibleHost = document.createElement('div')
    document.body.append(visibleHost)
    const host = createIframeHost({ visibleHost, assignIntervalMs: 50, heartbeatIntervalMs: 200, hostTabId: 123 })
    host.activateChat(makeChat('chat-1', ['role-1']), [makeRole('chat-1', 'role-1')])
    const iframe = host.getRoleFrame('chat-1', 'role-1')!
    const postMessage = vi.spyOn(iframe.contentWindow!, 'postMessage')
    ...
    expect(postMessage.mock.calls.length).toBe(callsAfterReady + 1)
    expect(postMessage).toHaveBeenLastCalledWith(
      { type: FRAME_ASSIGN_MESSAGE, chatId: 'chat-1', roleId: 'role-1', hostTabId: 123 },
      'https://gemini.google.com',
    )
    expect(host.getChatState('chat-1')[0].status).toBe('assigned')
  })

  it('uses a 10 second default assignment heartbeat after a role is ready', () => {
    const visibleHost = document.createElement('div')
    document.body.append(visibleHost)
    const host = createIframeHost({ visibleHost, assignIntervalMs: 50 })
    host.activateChat(makeChat('chat-1', ['role-1']), [makeRole('chat-1', 'role-1')])
  ...
  it('restarts fast assignment when a ready iframe reloads', () => {
    const visibleHost = document.createElement('div')
    document.body.append(visibleHost)
    const host = createIframeHost({ visibleHost, assignIntervalMs: 50, heartbeatIntervalMs: 500 })
    host.activateChat(makeChat('chat-1', ['role-1']), [makeRole('chat-1', 'role-1')])
    const iframe = host.getRoleFrame('chat-1', 'role-1')!
    const postMessage = vi.spyOn(iframe.contentWindow!, 'postMessage')
    ...
    expect(postMessage.mock.calls.length).toBe(callsAfterReady + 1)
    expect(host.getChatState('chat-1')[0].status).toBe('loading')
    vi.advanceTimersByTime(50)
    expect(postMessage.mock.calls.length).toBeGreaterThan(callsAfterReady + 1)
  })
